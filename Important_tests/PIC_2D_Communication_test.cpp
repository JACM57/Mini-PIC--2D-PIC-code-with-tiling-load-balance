...
// Particle structure: tag, charge, positions, momenta
struct Particle {
    int tag;
    double q;
    double x, y, z;
...
// Grid structure: Electric and Magnetic field components
struct Grid {
    double Ex, Ey, Ez;
    double Bx, By, Bz;
};

// TileInfo structure: contains tile metadata (globalID, row/col, sizes, current rank)
struct TileInfo {                           
...
// Tile structure definition: contains TileInfo along with other data (particles, grid cells)
struct Tile {
    TileInfo info;
...
struct RankInfo {
    ...
    vector<Tile> tiles; // List of tiles that belong to this rank *at the moment*
};
...
int main(int argc, char* argv[]) {
        ...
        if (rank == 0 || rank == 1 || rank == 2) {
            if ((int)info.tiles.size() > 3) {
                ...
                cout << "Rank " << rank << " is sending Tile with globalID="
                     << movingTile.info.globalID << " (currentRank=" << movingTile.info.currentRank << ") to Rank " << dest << endl << flush;
            ...
            for (int source = 0; source < 3; source++) {
                ...
                // Find tileRow,tileCol from inGID
                {
                    ...
                    newTile.info.tileRow = r_;
                    newTile.info.tileCol = c_;
                }
                ...
                cout << "Rank " << rank << " has received Tile with globalID="
                     << newTile.info.globalID << " (currentRank=" << newTile.info.currentRank << ") from Rank " << source << endl << flush;
            }
        }
    ...
    for (int i = 0; i < localTileCount; i++) {
        ...
        for (int d = 0; d < 8; d++) {
            ...
            int bufferSize = 0;
            if (d == 0 || d == 1)
                bufferSize = interior_ny * guard; // left/right (using interior_ny from main parameters)
            else if (d == 2 || d == 3)
                bufferSize = interior_nx * guard; // up/down (using interior_nx from main parameters)
            else
    ...
    for (int i = 0; i < localTileCount; i++) {
        Tile &tile = info.tiles[i];
        int gID = tile.info.globalID;    // # THIS CHANGED !
    ...
    for (int p = 0; p < size; p++) {
        ...
        if (rank == p) {
            ...
            for (int i = 0; i < (int)info.tiles.size(); i++) {
                ...
                for (int row = guard; row < guard + 2 && row < (tile.info.ny + guard); row++) {
                    for (int col = 0; col < guard; col++) {
                ...
                for (int row = 0; row < guard; row++) {
                    for (int col = guard; col < guard + 2 && col < (tile.info.nx + guard); col++) {
                        cout << tile.grid[row * totalX + col].Ex << " ";
                    }
                    cout << " | ";
                }
