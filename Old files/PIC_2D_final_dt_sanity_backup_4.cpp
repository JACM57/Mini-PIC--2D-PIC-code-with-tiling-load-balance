...
int main(int argc, char* argv[]) {
    ...
    double sim_time = 0.1;                                // Total simulation time [\omega_p^{-1}]
    double dt_courant = 1/(1/pow(dx,2)+(1/pow(dy,2)));  // Courant Condition
    double dt = 0.99 * dt_courant;                      // Time step size (respecting the CFL condition)
    // double dt = 0.01;
    int total_steps = static_cast<int>(sim_time / dt);  // Number of simulation steps
    int save_frequency = 1;                             // Frequency of saving simulation data
    ...
    for (int t = 0; t < numTiles; t++) {
        ...
        for (int j = 0; j < totalY; j++) {
            for (int i = 0; i < totalX; i++) {
                ...
                double x_Ey = (global_x_offset + i - guard + 0) * dx;
    ...
    // Save the initial state of the simulation (state at step 0)
