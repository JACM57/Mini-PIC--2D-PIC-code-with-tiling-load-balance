...
int main(int argc, char* argv[]) {
    ...
    // Initialize the local tiles and for each local tile t, compute its global ID
    for (int t = 0; t < numTiles; t++) {
        ...
        double val = rank + 0.1 * t; // for easy identification
        ...
        if (rank == 0 || rank == 1 || rank == 2) {
            if ((int)info.tiles.size() > 3) {
                ...
                cout << "Rank " << rank << " is sending Tile with globalID="
                     << movingTile.globalID << " to Rank " << dest << endl << flush;
            ...
            for (int source = 0; source < 3; source++) {
                ...
                cout << "Rank " << rank << " has received Tile with globalID="
                     << inGID << " from Rank " << source << endl << flush;
            }
        }
    ...
    vector< vector< vector<Grid> > >
        recvBuffers(localTileCount, vector< vector<Grid> >(8));
