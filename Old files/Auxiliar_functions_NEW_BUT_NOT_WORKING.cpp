...
vector<Grid> packSendBuffer(const Tile &tile, int d, int guard) {
    ...
    // d==0: Left boundary – use the first updated interior column.
    if (d == 0) {
        ...
        for (int row = 0; row < tile.info.ny; row++) {
            ...
            for (int col = 0; col < guard; col++) {
                int srcCol = guard + col; // first interior column is at index 'guard'
                sendBuffer[row * guard + col] = tile.grid[srcRow * totalX + srcCol];
            }
        }
    }
    // d==1: Right boundary – use the last updated interior column.
    else if (d == 1) {
        sendSize = tile.info.ny * guard;
        sendBuffer.resize(sendSize);
        int rightInteriorCol = guard + tile.info.nx - 1; // last updated interior column
        for (int row = 0; row < tile.info.ny; row++) {
            int srcRow = guard + row;
            for (int col = 0; col < guard; col++) {
                sendBuffer[row * guard + col] = tile.grid[srcRow * totalX + rightInteriorCol];
            }
        }
    }
    // d==2: Up boundary – use the first updated interior row.
    else if (d == 2) {
        sendSize = tile.info.nx * guard;
        sendBuffer.resize(sendSize);
        int topInteriorRow = guard; // first interior row
        for (int row = 0; row < guard; row++) {
            for (int col = 0; col < tile.info.nx; col++) {
                int srcRow = topInteriorRow;
                int srcCol = guard + col;
                sendBuffer[row * tile.info.nx + col] = tile.grid[srcRow * totalX + srcCol];
            }
        }
    }
    // d==3: Down boundary – use the last updated interior row.
    else if (d == 3) {
        sendSize = tile.info.nx * guard;
        sendBuffer.resize(sendSize);
        int bottomInteriorRow = guard + tile.info.ny - 1; // last interior row
        for (int row = 0; row < guard; row++) {
            for (int col = 0; col < tile.info.nx; col++) {
                int srcRow = bottomInteriorRow;
                int srcCol = guard + col;
                sendBuffer[row * tile.info.nx + col] = tile.grid[srcRow * totalX + srcCol];
            }
        }
    ...
    // d==5: Top-right corner.
    else if (d == 5) {
        sendSize = guard * guard;
        sendBuffer.resize(sendSize);
        int topInteriorRow = guard;
        int rightInteriorCol = guard + tile.info.nx - 1;
        for (int row = 0; row < guard; row++) {
            for (int col = 0; col < guard; col++) {
                int srcRow = topInteriorRow;
                int srcCol = rightInteriorCol;
                sendBuffer[row * guard + col] = tile.grid[srcRow * totalX + srcCol];
            }
        }
    }
    // d==6: Bottom-left corner.
    else if (d == 6) {
        sendSize = guard * guard;
        sendBuffer.resize(sendSize);
        int bottomInteriorRow = guard + tile.info.ny - 1;
        for (int row = 0; row < guard; row++) {
            for (int col = 0; col < guard; col++) {
                int srcRow = bottomInteriorRow;
                int srcCol = guard + col;
